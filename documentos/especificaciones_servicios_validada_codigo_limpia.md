...
El sistema valida que el documento no exista previamente. Tabla consultada: clientes.
Si no se selecciona ciudad, el sistema consulta una ciudad activa predeterminada. Tabla consultada: ciudades.
El sistema registra el nuevo cliente. Tabla registrada: clientes.
...
El sistema valida que el modelo seleccionado exista y esté activo. Tabla consultada: modelo_auto.
El sistema valida que la placa no exista previamente. Tabla consultada: vehiculos.
El sistema registra el vehículo. Tabla registrada: vehiculos.
...
Si la recepción proviene de reclamo, el sistema valida que el reclamo exista, esté activo y pertenezca a la sucursal del usuario. Tabla consultada: reclamo_servicio.
Si el reclamo requiere garantía, el sistema consulta el kilometraje de salida del servicio original. Tablas consultadas: reclamo_servicio, registro_servicio.
Si el servicio original no tiene kilometraje de salida registrado, el sistema no permite registrar la recepción.
El sistema registra la solicitud. Tabla registrada: recepcion_servicio.
Si la recepción proviene de un reclamo, el sistema actualiza el reclamo. Tabla actualizada: reclamo_servicio.
Si existen fotos adjuntas, el sistema guarda los archivos y registra sus rutas. Tabla registrada: recepcion_fotos.
...
El sistema consulta las recepciones de la sucursal actual. Tablas consultadas para listar recepciones: recepcion_servicio, clientes, vehiculos, modelo_auto, marcas, usuarios, recepcion_fotos.
El sistema muestra fecha, cliente, CI/RUC, vehículo, kilometraje, servicio, origen, prioridad, fotos, usuario y estado.
Ver Fotos
...
El sistema busca la recepción activa de la sucursal. Tabla consultada: recepcion_servicio.
El sistema cambia el estado de la recepción a Anulado. Tabla actualizada: recepcion_servicio.
Si la recepción provenía de un reclamo, el sistema devuelve el reclamo a estado activo. Tabla actualizada: reclamo_servicio.
...
Si faltan datos obligatorios de la recepción, cliente rápido o vehículo rápido, el sistema muestra datos incompletos y no guarda.
Si se intenta registrar un cliente o vehículo duplicado, el sistema muestra advertencia y no registra el duplicado.
Si se intenta registrar o buscar un vehículo sin cliente seleccionado, el sistema solicita seleccionar primero un cliente.
...
Si la recepción viene de reclamo y el reclamo no está seleccionado, disponible o vigente para garantía, el sistema cancela la recepción e informa el motivo.
Si la recepción no está activa o no pertenece a la sucursal del usuario, el sistema no permite anularla.
Si ocurre un error durante el registro o la anulación, el sistema revierte la operación.
...
El sistema consulta el detalle de la recepción seleccionada. Tablas consultadas: recepcion_servicio, clientes, vehiculos, modelo_auto, marcas.
...
El sistema valida que exista sucursal asociada al usuario en sesión.
El sistema valida que la recepción seleccionada pertenezca a la sucursal del usuario. Tabla consultada: recepcion_servicio.
El sistema valida que exista al menos un detalle del diagnóstico.
El sistema valida que la recepción siga disponible para diagnóstico. Tabla consultada: recepcion_servicio.
El sistema registra la cabecera del diagnóstico. Tabla registrada: diagnostico_servicio.
El sistema registra los detalles técnicos. Tabla registrada: diagnostico_detalle.
El sistema actualiza la recepción, dejándola en estado En proceso. Tabla actualizada: recepcion_servicio.
...
El sistema valida que el reclamo exista, pertenezca a la sucursal del usuario, esté en proceso y no tenga una OT activa. Tablas consultadas: reclamo_servicio, orden_trabajo.
El sistema registra la orden de trabajo con origen reclamo, sin presupuesto asociado y en estado pendiente de completar. Tabla registrada: orden_trabajo.
El diagnóstico asociado al reclamo se mantiene registrado como soporte técnico del reclamo; la OT asociada evita generar órdenes duplicadas.
...
El sistema consulta los diagnósticos de la sucursal actual. Tablas consultadas para listar diagnósticos: diagnostico_servicio, recepcion_servicio, clientes, vehiculos, modelo_auto, marcas, usuarios, equipo_trabajo, orden_trabajo.
El sistema muestra los diagnósticos encontrados con recepción, fecha, cliente, vehículo, servicio, origen, equipo, usuario registrador, estado y acciones.
El sistema muestra el botón Ver Detalle para todos los diagnósticos listados.
Si el diagnóstico habilita OT directa por reclamo y no tiene OT activa asociada, el sistema muestra el botón Generar OT por Reclamo. Tablas consultadas: diagnostico_servicio, recepcion_servicio, orden_trabajo.
Si el usuario tiene permiso de anulación y el diagnóstico no está anulado, el sistema muestra el botón Anular.
Si el diagnóstico ya está anulado, el sistema muestra la acción de anulación deshabilitada.
Si el usuario no tiene permiso de anulación, el sistema no muestra el botón Anular.
...
El sistema muestra datos de recepción, cliente, vehículo, equipo, usuario registrador, observaciones y detalles técnicos cargados.
El usuario presiona Anular.
El sistema emite mensaje de confirmación.
El usuario confirma la acción.
El sistema valida permisos de usuario.
El sistema valida el ID del diagnóstico.
El sistema verifica que el diagnóstico exista. Tabla consultada: diagnostico_servicio.
El sistema verifica que el diagnóstico no esté anulado. Tabla consultada: diagnostico_servicio.
El sistema verifica que el diagnóstico no tenga presupuesto de servicio activo asociado. Tabla consultada: presupuesto_servicio.
El sistema verifica si el diagnóstico pertenece a una recepción proveniente de reclamo. Tablas consultadas: diagnostico_servicio, recepcion_servicio.
Si el diagnóstico pertenece a una recepción proveniente de reclamo, el sistema valida que no exista una OT activa asociada a ese reclamo. Tabla consultada: orden_trabajo.
Si las validaciones son correctas, el sistema actualiza el estado del diagnóstico, dejándolo anulado. Tabla actualizada: diagnostico_servicio.
El sistema actualiza la recepción asociada, dejándola disponible nuevamente. Tabla actualizada: recepcion_servicio.
...
Si faltan recepción, equipo, sucursal o detalles técnicos, el sistema muestra datos incompletos y no guarda.
Si la recepción ya no está disponible o no pertenece a la sucursal del usuario, el sistema no permite guardar el diagnóstico.
Si falla el registro de la cabecera, los detalles o la actualización de la recepción, el sistema cancela la operación.
Si el diagnóstico no habilita OT directa por reclamo, el sistema no ofrece la generación automática de OT.
Si el usuario responde que no desea generar la OT ahora, el diagnóstico queda registrado.
Si no se cumplen permisos o condiciones para generar la OT por reclamo, el sistema cancela la generación e informa el motivo.
Si el diagnóstico ya está anulado, tiene presupuesto activo o tiene OT activa asociada por reclamo, el sistema no permite anularlo.
El usuario puede presionar Cancelar, y el sistema limpia el formulario.
...
El diagnóstico queda disponible para presupuesto de servicio cuando no tenga una OT activa asociada por reclamo, o para OT directa por reclamo cuando cumpla las condiciones correspondientes.
Si el usuario confirma una OT directa por reclamo, queda registrada una orden de trabajo con origen reclamo.
...
El sistema registra la cabecera de la promoción. Tabla registrada: promociones.
Si fueron seleccionados, el sistema registra los artículos asociados. Tabla registrada: promocion_producto.
...
El sistema actualiza la promoción. Tabla actualizada: promociones.
El sistema elimina la relación anterior de artículos asociados y registra la relación vigente. Tabla actualizada: promocion_producto.
...
El sistema actualiza el estado de la promoción. Tabla actualizada: promociones.
...
Si faltan datos obligatorios o los valores de tipo, importe o vigencia no son válidos, el sistema no permite guardar.
...
El usuario selecciona el alcance del descuento: total, producto o servicio.
...
Si el tipo es porcentaje, el sistema valida que no supere el cien por ciento.
El sistema valida que la fecha de inicio no sea mayor a la fecha fin.
Si el alcance no es total, producto o servicio, el sistema lo toma como descuento aplicado al total.
El sistema registra el descuento. Tabla registrada: descuentos.
Si fueron seleccionados, el sistema registra los clientes asociados. Tabla registrada: descuento_cliente.
El sistema emite mensaje de registro correcto.
...
Si elimina un cliente asociado, el sistema valida los datos recibidos y elimina la relación. Tabla actualizada: descuento_cliente.
El usuario guarda los cambios.
El sistema valida permisos para editar descuentos.
El sistema valida tipo, valor, alcance y vigencia.
El sistema actualiza el descuento. Tabla actualizada: descuentos.
Si se agregan clientes, el sistema registra las nuevas relaciones. Tabla registrada: descuento_cliente.
El sistema emite mensaje de actualización correcta.
...
Si faltan datos obligatorios o los valores de tipo, importe o vigencia no son válidos, el sistema no permite guardar.
Si el valor de aplica a no es total, producto o servicio, el sistema lo toma como descuento aplicado al total.
...
Los descuentos activos, vigentes, reutilizables y asociados al cliente quedan disponibles para aplicarse en presupuestos de servicio según su alcance: total del presupuesto, productos o servicios.
...
Descripción Básica
Este caso permite registrar, listar, aprobar, anular e imprimir presupuestos de servicio. El presupuesto puede generarse desde un diagnóstico disponible o como presupuesto preliminar para un cliente y vehículo. El sistema permite agregar servicios y repuestos, aplicar promociones vigentes, aplicar descuentos disponibles para el cliente según su alcance y calcular subtotal, descuentos y total final.
...
Debe existir un diagnóstico disponible para presupuesto, o un cliente y vehículo seleccionados para presupuesto preliminar. Si el diagnóstico proviene de reclamo, no debe tener una OT activa asociada al reclamo.
...
El sistema registra el detalle. Tabla registrada: presupuesto_detalleservicio.
El sistema registra promociones aplicadas cuando corresponda. Tabla registrada: presupuesto_promocion.
El sistema registra descuentos aplicados cuando corresponda, conservando el descuento, tipo, valor, alcance, base aplicada e importe aplicado cuando la estructura de la tabla lo permite. Tabla registrada: presupuesto_descuento.
Si proviene de diagnóstico, el sistema actualiza el diagnóstico como presupuestado. Tabla actualizada: diagnostico_servicio.
Si proviene de diagnóstico y se convirtió desde un preliminar, el sistema marca el preliminar convertido. Tabla actualizada: presupuesto_servicio.
...
El sistema valida que el presupuesto pueda anularse. Tabla consultada: presupuesto_servicio.
...
Los descuentos aplicados quedan registrados en presupuesto_descuento cuando corresponda, con su importe aplicado y, si la tabla cuenta con las columnas, con el alcance y la base usada para el cálculo.
...
Si la OT por reclamo no existe, pertenece a otra sucursal, no proviene de reclamo o no está pendiente de completar, el sistema no permite completarla desde esta pantalla.
Si el detalle de trabajos o repuestos no tiene formato válido, el sistema no permite completar la OT.
...
Flujo Alternativo:
Si el usuario no tiene permiso, el sistema muestra acceso denegado.
Si faltan registro de servicio, descripción, tipo de reclamo o detalles requeridos, el sistema muestra datos incompletos y no guarda.
Si el registro de servicio no existe, no pertenece a la sucursal o no permite reclamo, el sistema cancela la operación.
Si ya existe un reclamo activo o en proceso para el mismo tipo y registro, el sistema no permite duplicarlo.
Si los detalles seleccionados no pertenecen al registro o ya tienen reclamo activo, el sistema cancela la operación.
Si el reclamo no existe, no está activo, ya está anulado o tiene recepción generada, el sistema no permite anularlo.
Si ocurre un error durante el registro o anulación, el sistema revierte la transacción.
El usuario puede presionar Cancelar, y el sistema limpia el formulario.
...
Si se anula, se revierten los estados asociados cuando corresponda.
...
El sistema valida stock suficiente por insumo en la sucursal. Tabla consultada: stock.
El sistema registra la cabecera. Tabla registrada: salida_insumo.
El sistema registra el detalle. Tabla registrada: salida_insumo_detalle.
El sistema registra un movimiento de stock por cada insumo. Tabla registrada: movimientostock.
...
El usuario presiona Buscar.
El sistema consulta las salidas de insumos de la sucursal actual. Tablas consultadas para listar salidas: salida_insumo, empleados, usuarios.
...
El sistema verifica que la salida exista. Tabla consultada: salida_insumo.
El sistema verifica que la salida esté activa. Tabla consultada: salida_insumo.
El sistema verifica que la salida pertenezca a la sucursal del usuario. Tabla consultada: salida_insumo.
El sistema verifica que la salida tenga detalle válido. Tabla consultada: salida_insumo_detalle.
El sistema consulta el detalle de la salida. Tablas consultadas: salida_insumo_detalle, articulos.
El sistema registra un movimiento inverso de stock por cada insumo. Tabla registrada: movimientostock.
...
Si faltan empleado, insumos o cantidades válidas, el sistema muestra datos incompletos y no guarda.
Si un insumo no existe, no está activo, no es de tipo insumo o no tiene stock suficiente, el sistema cancela la operación.
Si la salida no existe, ya está anulada o pertenece a otra sucursal, el sistema no permite anularla.
...
Se registra movimiento de stock por salida de insumo en movimientostock.
Si se anula, la salida queda anulada y se revierten stock y movimiento.
