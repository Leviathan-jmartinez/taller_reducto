...
o	Si el cliente no existe, el usuario puede presionar el botón de nuevo cliente.
o	El usuario puede ingresar tipo de documento, documento, DV, nombre, apellido, teléfono, email, dirección, ciudad y estado civil.
o	Los campos obligatorios son documento y nombre.
o	La ciudad se busca escribiendo al menos dos caracteres y seleccionando una opción del listado.
o	Si no se selecciona ciudad, el sistema puede registrar el cliente usando una ciudad activa predeterminada.
o	El sistema valida los datos obligatorios del cliente.
o	El sistema valida que el usuario tenga permiso para registrar clientes.
o	El sistema valida que el documento no exista previamente. Tablas consultadas para nuevo cliente: clientes, ciudades.
o	El sistema registra el nuevo cliente.
...
o	Si el vehículo no existe, el usuario puede presionar el botón de nuevo vehículo.
o	El usuario puede ingresar modelo, color, placa, año, versión, transmisión, motor y tipo de vehículo.
o	Los campos obligatorios son modelo, color y placa.
o	El modelo se busca escribiendo al menos dos caracteres y seleccionando una opción del listado.
o	El sistema valida que el usuario tenga permiso para registrar vehículos.
o	El sistema valida cliente, modelo, color y placa.
o	El sistema valida que la placa no exista previamente. Tablas consultadas para nuevo vehículo: vehiculos, modelo_auto.
o	El sistema registra el vehículo en vehiculos.
...
o	El sistema valida que el usuario tenga permiso para registrar solicitudes de servicio.
o	El sistema valida que se haya seleccionado cliente.
o	El sistema valida que se haya seleccionado vehículo.
o	El sistema valida kilometraje.
o	El sistema valida observación.
o	Si la recepción proviene de reclamo, el sistema valida que exista un reclamo seleccionado.
o	Si el reclamo requiere garantía, el sistema valida las condiciones de garantía antes de registrar la recepción.
...
•	El sistema emite mensaje de confirmación.
•	El usuario confirma acción.
o	El sistema valida que el usuario tenga permiso para anular recepciones de servicio.
o	El sistema valida el ID de la recepción.
o	El sistema busca la recepción activa de la sucursal.
...
o	El sistema valida permisos de usuario.
o	El sistema valida que exista una recepción seleccionada.
o	El sistema valida que exista equipo de trabajo seleccionado.
o	El sistema valida que exista sucursal asociada.
o	El sistema valida que exista al menos un detalle del diagnóstico.
o	El sistema valida que la recepción siga disponible para diagnóstico.
...
•	El usuario confirma la acción.
o	El sistema valida permisos de usuario.
o	El sistema valida el ID del diagnóstico.
o	El sistema verifica que el diagnóstico exista.
o	El sistema verifica que el diagnóstico no esté anulado.
o	El sistema verifica que el diagnóstico no tenga presupuesto de servicio activo asociado.
o	El sistema verifica si el diagnóstico pertenece a una recepción proveniente de reclamo.
o	Si tiene reclamo asociado, el sistema valida que no exista una OT activa para ese reclamo. Tablas consultadas para anular: diagnostico_servicio, recepcion_servicio, presupuesto_servicio, orden_trabajo.
...
•	El usuario busca artículos activos.
•	El sistema consulta los artículos disponibles. Tablas consultadas: articulos.
...
•	El sistema emite mensaje de confirmación.
•	El usuario confirma la acción.
o	El sistema valida permisos para crear promociones.
o	El sistema valida nombre, tipo, valor y fechas.
o	El sistema valida que el tipo de promoción sea válido.
o	El sistema valida que el valor sea mayor a cero.
o	Si el tipo es porcentaje, el sistema valida que no supere el cien por ciento.
o	El sistema valida que la fecha inicio no sea mayor a la fecha fin.
•	El sistema registra la promoción en promociones.
•	El sistema registra los artículos asociados en promocion_producto.
...
•	El sistema consulta las promociones registradas. Tablas consultadas: promociones, sucursales.
...
o	El sistema valida permisos para editar promociones.
o	El sistema valida los mismos datos requeridos que en el registro.
•	El sistema actualiza la promoción.
•	El sistema actualiza la relación de artículos asociados.
...
•	El sistema cambia el estado de la promoción.
•	El sistema emite mensaje de operación correcta.

•	Flujo Alternativo:
Si el usuario no tiene permiso, el sistema muestra acceso no autorizado.
Si el tipo de promoción no es válido, el sistema no permite guardar.
Si el valor no es válido, el sistema no permite guardar.
Si la fecha inicio es mayor a la fecha fin, el sistema muestra advertencia.
Si no existen artículos activos, el usuario no puede asociar artículos.
Si ocurre un error durante el registro o edición, el sistema revierte la operación.

•	Post Condición
La promoción queda registrada o actualizada en promociones.
Los artículos asociados quedan registrados en promocion_producto.
La promoción queda disponible para ser aplicada a presupuestos cuando esté activa y vigente.
...
•	Descripción Básica
Permite registrar, listar, buscar y editar descuentos aplicables a presupuestos. Los descuentos pueden ser por porcentaje o por monto fijo, pueden tener rango de vigencia, pueden aplicar de forma general o a clientes específicos y pueden estar asociados a todas las sucursales o a una sucursal determinada.
...
•	Si el descuento aplica a clientes específicos, el usuario busca y selecciona los clientes.
•	El sistema consulta los clientes disponibles. Tablas consultadas: clientes.

Guardar
•	El usuario presiona Guardar.
•	El sistema emite mensaje de confirmación.
•	El usuario confirma la acción.
o	El sistema valida permisos para crear descuentos.
o	El sistema valida nombre, tipo y valor.
o	El sistema valida que el tipo sea porcentaje o monto fijo.
o	El sistema valida que el valor sea mayor a cero.
o	Si el tipo es porcentaje, el sistema valida que no supere el cien por ciento.
o	El sistema valida que la fecha inicio no sea mayor a la fecha fin.
•	El sistema registra el descuento en descuentos.
•	Si corresponde, el sistema registra los clientes asociados en descuento_cliente.
•	El sistema emite mensaje de registro correcto.

Editar
•	El usuario ingresa a Lista de Descuentos.
•	El sistema permite filtrar por estado, sucursal y vigencia.
•	El sistema consulta los descuentos registrados. Tablas consultadas: descuentos, sucursales.
•	El usuario presiona Editar.
•	El sistema carga los datos del descuento y los clientes asociados.
•	El usuario modifica los datos permitidos.
•	El usuario guarda los cambios.
o	El sistema valida permisos para editar descuentos.
o	El sistema valida los mismos datos requeridos que en el registro.
•	El sistema actualiza el descuento.
•	El sistema actualiza la relación de clientes asociados.
•	El sistema emite mensaje de actualización correcta.

•	Flujo Alternativo:
Si el usuario no tiene permiso, el sistema muestra acceso no autorizado.
Si el tipo de descuento no es válido, el sistema no permite guardar.
Si el valor no es válido, el sistema no permite guardar.
Si la fecha inicio es mayor a la fecha fin, el sistema muestra advertencia.
Si el descuento aplica a clientes específicos y no se seleccionan clientes, el sistema no registra relaciones de clientes.
Si ocurre un error durante el registro o edición, el sistema revierte la operación.

•	Post Condición
El descuento queda registrado o actualizado en descuentos.
Los clientes asociados quedan registrados en descuento_cliente cuando corresponda.
El descuento queda disponible para presupuestos cuando esté vigente y activo.
...
•	El sistema carga los datos del diagnóstico, cliente, vehículo y detalle técnico.
•	El sistema identifica si el presupuesto proviene de diagnóstico.
•	El usuario ingresa fecha de vencimiento.
•	El usuario puede agregar servicios o repuestos al detalle.
...
•	El sistema puede aplicar promociones vigentes sobre los artículos seleccionados.
•	El sistema puede consultar descuentos disponibles para el cliente. Tablas consultadas: descuentos, descuento_cliente.
...
•	El sistema puede buscar presupuestos preliminares existentes para el cliente y vehículo.
...
•	El sistema emite mensaje de confirmación.
•	El usuario confirma la acción.
o	El sistema valida permisos para registrar presupuestos.
o	El sistema valida fecha de vencimiento.
o	Si el presupuesto proviene de diagnóstico, el sistema valida que exista diagnóstico seleccionado.
o	Si el presupuesto es preliminar, el sistema valida cliente y vehículo.
o	El sistema valida que exista al menos un detalle.
o	El sistema valida importes y cantidades.
o	El sistema valida stock disponible para productos cuando corresponda.
...
•	El sistema emite mensaje de confirmación.
•	El usuario confirma la acción.
o	El sistema valida permisos para aprobar presupuestos.
o	El sistema valida que el presupuesto pertenezca a la sucursal del usuario.
o	El sistema valida que el presupuesto esté en estado pendiente.
...
•	El sistema emite mensaje de confirmación.
•	El usuario confirma la acción.
o	El sistema valida permisos para anular presupuestos.
o	El sistema valida el ID del presupuesto.
o	El sistema valida que el presupuesto pueda anularse.
o	El sistema valida que no tenga una orden de trabajo activa asociada.
...
•	El sistema emite mensaje de confirmación.
•	El usuario confirma acción.
o	El sistema valida permisos para generar OT.
o	El sistema valida que exista presupuesto seleccionado.
o	El sistema valida que exista equipo y técnico responsable.
o	El sistema valida que el presupuesto esté aprobado.
o	El sistema valida que el presupuesto pertenezca a la sucursal del usuario.
o	El sistema valida que el presupuesto no tenga una OT activa.
o	El sistema valida que el presupuesto provenga de diagnóstico cuando corresponda para OT desde presupuesto.
...
•	El sistema consulta los datos necesarios para completar la OT.
•	Tablas consultadas para completar OT: orden_trabajo, recepcion_servicio, clientes, vehiculos, modelo_auto, diagnostico_servicio.
•	El sistema muestra información del vehículo, diagnóstico asociado y detalle técnico.
...
•	El usuario presiona Guardar.
o	El sistema valida permisos de usuario.
o	El sistema valida datos obligatorios: OT, equipo y técnico.
o	El sistema valida que exista al menos un trabajo o repuesto.
o	El sistema valida existencia, sucursal, origen reclamo y estado pendiente de completar.
o	Si existen repuestos, el sistema consulta stock por artículo y sucursal.
o	El sistema valida stock suficiente por cada repuesto.
•	El sistema registra trabajos y repuestos en orden_trabajo_detalle.
...
•	El usuario confirma la acción.
o	El sistema valida permisos de usuario.
o	El sistema verifica que la OT exista, pertenezca a la sucursal del usuario y se encuentre en estado permitido para anular. Tabla consultada: orden_trabajo.
o	El sistema valida que la OT no tenga servicio registrado activo. Tabla consultada: registro_servicio.
...
•	Si la OT proviene de reclamo, el sistema reactiva el flujo asociado al reclamo. Tablas actualizadas: reclamo_servicio, diagnostico_servicio.
...
Si se anula la OT, queda en estado Anulada y se revierte el estado del presupuesto o del flujo de reclamo según corresponda.
...
•	El sistema emite mensaje de confirmación.
•	El usuario confirma la acción.
o	El sistema valida permisos para registrar.
o	El sistema valida datos obligatorios.
o	El sistema valida que la OT esté activa y que pertenezca a la sucursal del usuario. Tabla consultada: orden_trabajo.
o	El sistema valida que no exista un registro de servicio activo previo para la OT. Tabla consultada: registro_servicio.
o	El sistema valida que la OT tenga detalle.
o	Si se agregan insumos, el sistema valida stock suficiente.
...
•	El sistema emite mensaje de confirmación.
•	El usuario confirma la acción.
o	El sistema valida permiso de anulación.
o	El sistema valida el registro seleccionado.
o	El sistema verifica que el registro exista, esté activo y pertenezca a la sucursal del usuario.
o	Tablas consultadas para validar anulación: registro_servicio, orden_trabajo.
...
o	El sistema valida permisos para registrar.
o	El sistema valida datos obligatorios.
o	El sistema valida que exista el registro de servicio.
o	El sistema valida que el registro de servicio pertenezca a la sucursal del usuario. Tabla consultada: registro_servicio.
o	El sistema valida que no exista otro reclamo activo o en proceso del mismo tipo para el mismo registro. Tabla consultada: reclamo_servicio.
o	El sistema valida que los detalles seleccionados pertenezcan al registro de servicio.
o	El sistema valida que los detalles seleccionados no tengan reclamo activo.
o	Si requiere garantía, el sistema valida vigencia por fecha y kilometraje cuando corresponda.
...
•	El usuario confirma la acción.
o	El sistema valida permiso de anulación.
o	El sistema valida el reclamo seleccionado.
o	El sistema verifica que el reclamo exista, esté activo y pertenezca a la sucursal del usuario. Tabla consultada: reclamo_servicio.
o	El sistema verifica que el reclamo no tenga recepción generada. Tabla consultada: recepcion_servicio.
•	El sistema anula el reclamo. Tabla actualizada: reclamo_servicio.
•	El sistema verifica si quedan reclamos activos o en proceso para el mismo registro de servicio. Tabla consultada: reclamo_servicio.
•	Si no quedan reclamos activos, el sistema devuelve el registro de servicio a estado activo. Tabla actualizada: registro_servicio.
...
•	El sistema emite mensaje de confirmación.
•	El usuario confirma la acción.
o	El sistema valida permisos para registrar salida de insumos.
o	El sistema valida que exista una sesión válida y una sucursal asociada.
o	El sistema valida que se haya seleccionado empleado responsable.
o	El sistema valida que exista al menos un insumo en el detalle.
o	El sistema valida que cada insumo sea válido, esté activo y sea de tipo insumo.
o	El sistema valida que cada cantidad sea mayor a cero.
o	El sistema valida stock suficiente por insumo en la sucursal.
...
•	El sistema emite mensaje de confirmación.
•	El usuario confirma la acción.
o	El sistema valida permisos para anular salidas de insumos.
o	El sistema valida que exista una sesión válida y una sucursal asociada.
o	El sistema valida el ID de la salida.
o	El sistema verifica que la salida exista.
o	El sistema verifica que la salida esté activa.
o	El sistema verifica que la salida pertenezca a la sucursal del usuario.
o	El sistema verifica que la salida tenga detalle válido.
